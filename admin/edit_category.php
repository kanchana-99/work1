                ...
                <div class="card mb-12">
                  ...
                  <form action="edit-category-api.php" method="post" >
                    <?php $editid = $_GET['id'];
                        ...
                        $query->bindParam(':cid',$editid,PDO::PARAM_STR); //ใส่ตอนมีเงื่อนไข where
                        $query->execute();
                        $resalts = $query->fetchAll(PDO::FETCH_OBJ);

                        if ($query->rowCount() >0) {    //นับแถว
                            foreach($resalts as $row){

                    ?>
                        <input type="hidden" name="cid" id="cid" value="<?php echo $editid; ?>">
                                <div class="form-group">
                                  <label for="cat_name">ประเภทสินค้า:</label>
                                  <input type="text" class="form-control" id="cat_name" placeholder="Enter Category" name="cat_name" 
                                  required value="<?php echo $row->cat_name; ?>">
                                </div>
                    <?php
                                }
                            }
